...
public class AsylumCaseUtils {
    ...
    public static boolean isInternalCase(AsylumCase asylumCase) {
        return asylumCase.read(IS_ADMIN, YesOrNo.class).map(isAdmin -> YES == isAdmin)
            .orElse(false);
    }
    ...
    public static boolean inCountryAppeal(AsylumCase asylumCase) {
        return asylumCase.read(APPELLANT_IN_UK, YesOrNo.class)
            .map(value -> value.equals(YesOrNo.YES)).orElse(false);
    }
    ...
    public static boolean isAriaMigrated(AsylumCase asylumCase) {
        return asylumCase.read(IS_ARIA_MIGRATED, YesOrNo.class).map(isAdmin -> YES == isAdmin)
            .orElse(false);
    }

    public static Optional<FtpaDecisionOutcomeType> getFtpaDecisionOutcomeType(
        ...
        return asylumCase.read(FTPA_RESPONDENT_RJ_DECISION_OUTCOME_TYPE,
    ...
    public static String getDetentionFacilityName(AsylumCase asylumCase) {
        ...
        switch (detentionFacility) {
            ...
            case "other":
                return asylumCase.read(OTHER_DETENTION_FACILITY_NAME,
                        OtherDetentionFacilityName.class)
                    .orElseThrow(() -> new RequiredFieldMissingException(
                        "Other detention facility name is missing")).getOther();
            default:
                throw new RequiredFieldMissingException("Detention Facility is missing");
        }
    }

    public static DocumentWithMetadata getLetterForNotification(AsylumCase asylumCase,
    ...
    public static List<IdValue<DocumentWithMetadata>> getAddendumEvidenceDocuments(
        AsylumCase asylumCase) {
    ...
    public static Optional<IdValue<DocumentWithMetadata>> getLatestAddendumEvidenceDocument(
        AsylumCase asylumCase) {
        ...
        Optional<IdValue<DocumentWithMetadata>> optionalLatestAddendum = addendums.stream()
            .findFirst();

        return optionalLatestAddendum.isEmpty() ? Optional.empty()
            : Optional.of(optionalLatestAddendum.get());
    }
    ...
    public static ImmutablePair<String, String> getApplicantAndRespondent(AsylumCase asylumCase,
        Function<AsylumCase, ApplyForCosts> retrieveApplyForCosts) {
    ...
    public static ApplyForCosts getApplicationById(AsylumCase asylumCase,
        AsylumCaseDefinition definition) {
        ...
        Optional<List<IdValue<ApplyForCosts>>> maybeApplyForCosts = asylumCase.read(
            APPLIES_FOR_COSTS);
        ...
        return maybeApplyForCosts
            ...
            .orElseThrow(() -> new IllegalStateException(
                "Apply for costs with id " + applicationId + " not found"))
            .getValue();
    }

    public static boolean isLoggedUserIsHomeOffice(AsylumCase asylumCase,
        Function<AsylumCase, ApplyForCosts> retrieveApplyForCosts) {
    ...
    public static YesOrNo isAppellantInUK(AsylumCase asylumCase) {
        return asylumCase.read(AsylumCaseDefinition.APPELLANT_IN_UK, YesOrNo.class)
            .orElse(YesOrNo.NO);
    }
    ...
    public static List<String> getAppellantAddressAsList(final AsylumCase asylumCase) {
        ...
        appellantAddressAsList.add(address.getAddressLine1().orElseThrow(
            () -> new IllegalStateException("appellantAddress line 1 is not present")));
        ...
        appellantAddressAsList.add(address.getPostTown().orElseThrow(
            () -> new IllegalStateException("appellantAddress postTown is not present")));
        appellantAddressAsList.add(address.getPostCode().orElseThrow(
            () -> new IllegalStateException("appellantAddress postCode is not present")));
    ...
    public static List<String> getLegalRepresentativeAddressAsList(final AsylumCase asylumCase) {
        ...
        legalRepAddressAsList.add(address.getAddressLine1()
            .orElseThrow(() -> new IllegalStateException("legalRepAddress line 1 is not present")));
        String addressLine2 = address.getAddressLine2().orElse(null);
        String addressLine3 = address.getAddressLine3().orElse(null);
        ...
        legalRepAddressAsList.add(address.getPostTown().orElseThrow(
            () -> new IllegalStateException("legalRepAddress postTown is not present")));
        legalRepAddressAsList.add(address.getPostCode().orElseThrow(
            () -> new IllegalStateException("legalRepAddress postCode is not present")));
    ...
    public static List<String> getLegalRepresentativeAddressOocAsList(final AsylumCase asylumCase) {
        ...
        String addressLine1 = asylumCase
            ...
            .orElseThrow(
                () -> new IllegalStateException("Ooc Legal Rep Address line 1 is not present"));

        String addressLine2 = asylumCase
            .read(OOC_ADDRESS_LINE_2, String.class)
            .orElseThrow(
                () -> new IllegalStateException("Ooc Legal Rep Address line 2 is not present"));
        ...
        NationalityGovUk oocLrCountryGovUkAdminJ = NationalityGovUk.valueOf(asylumCase
            ...
            .orElseThrow(() -> new IllegalStateException("oocLrCountryGovUkAdminJ is not present"))
            .getCode());
    ...
    public static String calculateFeeDifference(String originalFeeTotal, String newFeeTotal) {
        try {
            ...
            BigDecimal originalFee = new BigDecimal(
                String.valueOf(Double.parseDouble(originalFeeTotal) / 100));
            BigDecimal newFee = new BigDecimal(
                String.valueOf(Double.parseDouble(newFeeTotal) / 100));
    ...
    public static Set<String> getAppellantAddressInCountryOrOoc(final AsylumCase asylumCase) {
        return inCountryAppeal(asylumCase) ? Collections.singleton(
            getAppellantAddressAsList(asylumCase).stream()
    ...
    public static Set<String> getLegalRepAddressInCountryOrOoc(final AsylumCase asylumCase) {
        return legalRepInCountryAppeal(asylumCase) ? Collections.singleton(
            getLegalRepresentativeAddressAsList(asylumCase).stream()
                .map(item -> item.replaceAll("\\s", "")).collect(Collectors.joining("_"))) :
    ...
    public static String getLegalRepEmailInternalOrLegalRepJourney(final AsylumCase asylumCase) {
        if (isInternalCase(asylumCase) && hasBeenSubmittedAsLegalRepresentedInternalCase(
            asylumCase)) {
            return asylumCase.read(LEGAL_REP_EMAIL, String.class)
                .orElseThrow(() -> new IllegalStateException("legalRepEmail is not present"));
        } else if (isInternalCase(asylumCase) && !hasBeenSubmittedAsLegalRepresentedInternalCase(
            asylumCase)) {
            return StringUtils.EMPTY;
        } else {
            return asylumCase.read(LEGAL_REPRESENTATIVE_EMAIL_ADDRESS, String.class).orElseThrow(
                () -> new IllegalStateException("legalRepresentativeEmailAddress is not present"));
        }
    }

    public static String getLegalRepEmailInternalOrLegalRepJourneyNonMandatory(
        final AsylumCase asylumCase) {
        return
            isInternalCase(asylumCase) && hasBeenSubmittedAsLegalRepresentedInternalCase(asylumCase)
                ? asylumCase.read(LEGAL_REP_EMAIL, String.class).orElse("")
    ...
    public static List<String> getAppellantOrLegalRepAddressLetterPersonalisation(
        AsylumCase asylumCase) {
