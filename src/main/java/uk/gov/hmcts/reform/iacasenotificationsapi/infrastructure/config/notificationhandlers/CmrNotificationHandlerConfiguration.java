    ...
    public PreSubmitCallbackHandler<AsylumCase> cmrInPersonDetainedInPrisonIrcListedNotificationHandler(
        ...
        return new NotificationHandler(
            (callbackStage, callback) -> {
                ...
                log.info(
                        "---------cmrInPersonDetainedInPrisonIrcListedNotificationHandler: {} {}",
    ...
    public PreSubmitCallbackHandler<AsylumCase> cmrListingLrManualDetainedInPrisonOrIrcNotificationHandler(
        ...
        return new NotificationHandler(
            (callbackStage, callback) -> {
                AsylumCase asylumCase = callback.getCaseDetails().getCaseData();
                log.info(
                        "---------cmrListingLrManualDetainedInPrisonOrIrcNotificationHandler: {} {}",
                        callback.getCaseDetails().getId(),
                        callbackStage == PreSubmitCallbackStage.ABOUT_TO_SUBMIT
                            && CMR_LISTING.equals(callback.getEvent())
                            && isCmrHearingInPersonOrRemote(asylumCase)
    ...
    public PreSubmitCallbackHandler<AsylumCase> cmrRelistingNonDetainedLegallyRepresentedManualAppeal(
        ...
        return new NotificationHandler(
                (callbackStage, callback) -> {
                    AsylumCase asylumCase = callback.getCaseDetails().getCaseData();
                    log.info(
                            "---------cmrRelistingNonDetainedLegallyRepresentedManualAppeal: {} {}",
                            callback.getCaseDetails().getId(),
                            callbackStage == PreSubmitCallbackStage.ABOUT_TO_SUBMIT && CMR_RE_LISTING.equals(callback.getEvent())
                                    && isCmrHearingInPersonOrRemote(asylumCase)
                                    && hasBeenSubmittedAsLegalRepresentedInternalCase(asylumCase)
    ...
    public PreSubmitCallbackHandler<AsylumCase> cmrReListingLegalRepDigitalDetainedInOtherNotificationHandler(
        ...
        return new NotificationHandler(
                (callbackStage, callback) -> {
                    AsylumCase asylumCase = callback.getCaseDetails().getCaseData();
                    log.info(
                            "---------cmrReListingLegalRepDigitalDetainedInOtherNotificationHandler: {} {}",
                            callback.getCaseDetails().getId(),
                            callbackStage == PreSubmitCallbackStage.ABOUT_TO_SUBMIT
                                    && CMR_RE_LISTING.equals(callback.getEvent())
                                    && isCmrHearingInPersonOrRemote(asylumCase)
    ...
    public PreSubmitCallbackHandler<AsylumCase> cmrListingAipManualDetainedInPrisonOrIrcNotificationHandler(
        ...
        return new NotificationHandler(
            (callbackStage, callback) -> {
                AsylumCase asylumCase = callback.getCaseDetails().getCaseData();
                log.info(
                        "---------cmrListingAipManualDetainedInPrisonOrIrcNotificationHandler: {} {}",
                        callback.getCaseDetails().getId(),
                        callbackStage == PreSubmitCallbackStage.ABOUT_TO_SUBMIT
                                && CMR_LISTING.equals(callback.getEvent())
                                && isCmrHearingInPersonOrRemote(asylumCase)
