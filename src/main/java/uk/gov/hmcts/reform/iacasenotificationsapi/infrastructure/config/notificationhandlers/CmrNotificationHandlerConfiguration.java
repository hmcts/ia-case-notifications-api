...
public class CmrNotificationHandlerConfiguration {
    ...
    @Bean
    public PreSubmitCallbackHandler<AsylumCase> cmrListingAipManualNonDetainedNotificationHandler(
        @Qualifier("aipManualNonDetainedCmrListingNotificationGenerator") List<NotificationGenerator> notificationGenerators
    ) {
    ...
    @Bean
    public PreSubmitCallbackHandler<AsylumCase> remoteCmrListingLrNotificationHandler(
            @Qualifier("remoteCmrListingLrNotificationGenerator") List<NotificationGenerator> notificationGenerators
    ) {
    ...
    public PreSubmitCallbackHandler<AsylumCase> remoteCmrRelistingLrPostalNotificationHandler(
            @Qualifier("remoteCmrRelistingLrPostalNotificationGenerator") List<NotificationGenerator> notificationGenerators
    ) {
    ...
    private boolean isAipManualOtherOrNonDetainedCmrRelisting(Callback<AsylumCase> callback, AsylumCase asylumCase) {
        return CMR_RE_LISTING.equals(callback.getEvent())
            && isCmrHearingInPersonOrRemote(asylumCase)
            && hasBeenSubmittedByAppellantInternalCase(asylumCase)
            && !isDetainedInOneOfFacilityTypes(asylumCase, IRC, PRISON);
