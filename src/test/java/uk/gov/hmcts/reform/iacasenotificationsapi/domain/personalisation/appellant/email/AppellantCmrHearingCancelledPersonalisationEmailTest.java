...
class AppellantCmrHearingCancelledPersonalisationEmailTest {
    ...
    private AppellantCmrHearingCancelledPersonalisationEmail personalisation;
    ...
    @BeforeEach
    void setUp() {
        personalisation = new AppellantCmrHearingCancelledPersonalisationEmail(
                TEMPLATE_ID, recipientsFinder, dateTimeExtractor, hearingDetailsFinder);

        when(callback.getCaseDetails()).thenReturn(caseDetails);
        when(caseDetails.getCaseData()).thenReturn(asylumCase);

        when(emailAddressFinder.getLegalRepEmailAddress(asylumCase)).thenReturn(EMAIL);
    }
    ...
    void should_return_reference_id() {
        assertThat(personalisation.getReferenceId(12345L))
                .isEqualTo("12345_CMR_HEARING_CANCELLED_APPELLANT_EMAIL");
    }
