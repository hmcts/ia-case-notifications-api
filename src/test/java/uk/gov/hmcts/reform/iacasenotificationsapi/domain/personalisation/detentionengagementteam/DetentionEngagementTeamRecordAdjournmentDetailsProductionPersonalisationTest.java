...
class DetentionEngagementTeamRecordAdjournmentDetailsProductionPersonalisationTest {

    private static final String TEMPLATE_ID = "template-id";
    private static final String EMAIL = "detained@email.com";
    private static final String SUBJECT_PREFIX = "Appeal:";
    private static final String PRISON = "prison";

    private DetentionEngagementTeamRecordAdjournmentDetailsProductionPersonalisation personalisation;
    ...
    @BeforeEach
    void setUp() {
        personalisation = new DetentionEngagementTeamRecordAdjournmentDetailsProductionPersonalisation(
                TEMPLATE_ID,
    ...
    @Test
    void should_return_empty_recipient_if_not_detained_or_invalid_facility() {
    ...
    @Test
    void should_return_personalisation_with_all_values() {
        when(callback.getCaseDetailsBefore()).thenReturn(Optional.of(caseDetailsBefore));
        when(caseDetailsBefore.getCaseData()).thenReturn(asylumCaseBefore);

        when(hearingDetailsFinder.getHearingDateTime(asylumCaseBefore)).thenReturn("2025-08-01T10:30");
        when(dateTimeExtractor.extractHearingDate("2025-08-01T10:30")).thenReturn("01-08-2025");
        when(dateTimeExtractor.extractHearingTime("2025-08-01T10:30")).thenReturn("10:30");
        when(hearingDetailsFinder.getHearingCentreAddress(asylumCaseBefore)).thenReturn("Centre Address");

        when(asylumCase.read(DETENTION_FACILITY, String.class)).thenReturn(Optional.of(PRISON));
        when(asylumCase.read(PRISON_NOMS, PrisonNomsNumber.class)).thenReturn(Optional.of(prisonNomsNumber));
        when(prisonNomsNumber.getPrison()).thenReturn("NOMS123");

        when(asylumCase.read(APPEAL_REFERENCE_NUMBER, String.class)).thenReturn(Optional.of("AR123"));
        ...
        when(asylumCase.read(DETENTION_BUILDING, String.class)).thenReturn(Optional.of("Detention Building"));
        ...
        assertThat(personalisationMap)
                ...
                .containsEntry("appealReferenceNumber", "AR123")
                ...
                .containsEntry("nomsRef", "NOMS Ref: NOMS123")
                .containsEntry("hearingDate", "01-08-2025")
                .containsEntry("hearingTime", "10:30")
                .containsEntry("hearingCentreAddress", "Centre Address")
                .containsEntry("detentionBuilding", "Detention Building");
    }

    @Test
    void should_return_empty_defaults_if_case_details_before_missing() {
        ...
        assertThat(personalisationMap.get("hearingDate")).isEqualTo("");
        assertThat(personalisationMap.get("hearingTime")).isEqualTo("");
        assertThat(personalisationMap.get("hearingCentreAddress")).isEqualTo("");
        assertThat(personalisationMap.get("nomsRef")).isEqualTo("");
    }
