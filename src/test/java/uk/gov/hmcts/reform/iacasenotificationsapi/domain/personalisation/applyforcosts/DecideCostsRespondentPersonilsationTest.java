...
class DecideCostsRespondentPersonalisationTest {
    @Mock
    ...
    private DecideCostsRespondentPersonilsation decideCostsRespondentPersonalisation;

    @BeforeEach
    void setup() {
        decideCostsRespondentPersonalisation = new DecideCostsRespondentPersonilsation(
            templateId,
        ...
        Map<String, String> decideCostsRespondentPersonalisationTemplate = new HashMap<>();

        decideCostsRespondentPersonalisationTemplate.put("appellantGivenNames", appellantGivenNames);
        decideCostsRespondentPersonalisationTemplate.put("appellantFamilyName", appellantFamilyName);
        decideCostsRespondentPersonalisationTemplate.put("appealReferenceNumber", appealReferenceNumber);
        decideCostsRespondentPersonalisationTemplate.put("linkToOnlineService", iaExUiFrontendUrl);
        decideCostsRespondentPersonalisationTemplate.put("applicationId", newestApplicationCreatedNumber);
        decideCostsRespondentPersonalisationTemplate.put("appliedCostsType", "Wasted");
        decideCostsRespondentPersonalisationTemplate.put("ariaListingReference", ariaListingReference);

        when((customerServicesProvider.getCustomerServicesTelephone())).thenReturn(customerServicesTelephone);
        when((customerServicesProvider.getCustomerServicesEmail())).thenReturn(customerServicesEmail);
        when(personalisationProvider.getApplyForCostsPesonalisation(asylumCase)).thenReturn(decideCostsRespondentPersonalisationTemplate);
    ...
    void should_return_given_template_id() {
        assertEquals(templateId, decideCostsRespondentPersonalisation.getTemplateId(asylumCase));
    }

    @Test
    void should_return_given_reference_id() {
        assertEquals(caseId + "_RESPOND_TO_COSTS_RESPONDENT_EMAIL",
            decideCostsRespondentPersonalisation.getReferenceId(caseId));
    }
    ...
    void should_return_given_email_address(List<IdValue<ApplyForCosts>> applyForCostsList) {
        ...
        if (applyForCostsList.get(0).getValue().getApplyForCostsApplicantType().equals(homeOffice)) {
            assertTrue(decideCostsRespondentPersonalisation.getRecipientsList(asylumCase).contains(legalRepEmailAddress));
        } else {
            assertTrue(decideCostsRespondentPersonalisation.getRecipientsList(asylumCase).contains(homeOfficeEmailAddress));
        }
    }
    ...
    void should_throw_exception_on_personalisation_when_case_is_null() {
        ...
        assertThatThrownBy(() -> decideCostsRespondentPersonalisation.getPersonalisation((AsylumCase) null))
    ...
    void should_return_personalisation_when_all_information_given(List<IdValue<ApplyForCosts>> applyForCostsList) {
        ...
        Map<String, String> personalisation = decideCostsRespondentPersonalisation.getPersonalisation(asylumCase);
