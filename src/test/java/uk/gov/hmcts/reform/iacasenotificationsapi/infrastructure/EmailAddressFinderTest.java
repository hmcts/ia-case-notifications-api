...
public class EmailAddressFinderTest {
    ...
    @Test
    public void should_fallback_to_hearing_centre_when_list_case_hearing_centre_is_shared_for_ftpa_ho_email() {
        when(asylumCase.read(LIST_CASE_HEARING_CENTRE, HearingCentre.class)).thenReturn(Optional.of(HENDON));
        when(asylumCase.read(HEARING_CENTRE, HearingCentre.class)).thenReturn(Optional.of(TAYLOR_HOUSE));
        when(homeOfficeFtpaEmailAddresses.get(TAYLOR_HOUSE)).thenReturn("hc-taylorhouse@example.com");
        assertEquals("hc-taylorhouse@example.com",
                emailAddressFinder.getListCaseFtpaHomeOfficeEmailAddress(asylumCase));
    }

    @Test
    public void should_throw_when_shared_hearing_centre_and_hearing_centre_is_empty_for_ftpa_ho_email() {
        when(asylumCase.read(LIST_CASE_HEARING_CENTRE, HearingCentre.class)).thenReturn(Optional.of(HENDON));
        when(asylumCase.read(HEARING_CENTRE, HearingCentre.class)).thenReturn(Optional.empty());
    ...
    @Test
    public void should_throw_when_shared_hearing_centre_and_hearing_centre_is_empty() {
        when(asylumCase.read(LIST_CASE_HEARING_CENTRE, HearingCentre.class)).thenReturn(Optional.of(HENDON));
        when(asylumCase.read(HEARING_CENTRE, HearingCentre.class)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> emailAddressFinder.getListCaseHearingCentreEmailAddress(asylumCase))
                .isExactlyInstanceOf(IllegalStateException.class)
                .hasMessage("hearingCentre is not present");
    }
