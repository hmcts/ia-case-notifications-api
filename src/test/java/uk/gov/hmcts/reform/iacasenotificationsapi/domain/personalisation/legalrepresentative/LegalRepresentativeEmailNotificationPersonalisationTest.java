...
class LegalRepresentativeEmailNotificationPersonalisationTest {
    ...
    public void should_return_given_email_address_from_asylum_case() {
        ...
        when(asylumCase.read(CHANGE_ORGANISATION_REQUEST_FIELD)).thenReturn(Optional.empty());
    ...
    @Test
    public void should_return_empty_email_address_for_change_org_request_field() {

        Value caseRole =
            new Value("[LEGALREPRESENTATIVE]", "Legal Representative");

        when(asylumCase.read(CHANGE_ORGANISATION_REQUEST_FIELD)).thenReturn(Optional.of(new ChangeOrganisationRequest(
